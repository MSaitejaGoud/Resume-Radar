...
def render_header():
    # Header with embedded theme toggle button
...
def render_results(results):
    ...
    with col_left:
        ...
        if results["jd_summary"]["requirements"]:
            for req in results["jd_summary"]["requirements"]:
                st.markdown(f"• {req}")
    ...
    with col_right:
        ...
        st.markdown(f"- **Total JD Skills:** {results.get('total_jd_skills', 0)}\n- **Skills You Have:** {len(results['matched_skills'])}\n- **Skills to Learn:** {len(results['missing_skills'])}\n- **Match Rating:** {rating}")
